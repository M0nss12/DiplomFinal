// seeder.js (Supabase JS client, real Storage URLs, chunked inserts)
...
/**
 * Вставка с повторными попытками при fetch-ошибках.
 */
/**
 * Вставка записей порциями с повторными попытками при сетевых сбоях.
 * @param {string} table
 * @param {Array} records
 * @param {number} chunkSize
 * @param {number} delayMs
 * @param {number} retries
 * @param {string} selectCols — столбцы для .select() (по умолчанию 'id')
 */
async function insertInBatches(table, records, chunkSize = 10, delayMs = 400, retries = 3, selectCols = 'id') {
  const results = [];
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        if (err?.message?.includes('fetch') || err?.message?.includes('ECONNRESET')) {
          console.warn(`⚠️  Сетевая ошибка при вставке в ${table} (попытка ${attempt}/${retries})`);
...
// ---------------------------------------------------------------------------
// STORAGE BASE URLs (публичные бакеты)
// ---------------------------------------------------------------------------
...
async function seed() {
  ...
  const cityIds = (await insertInBatches('cities', citiesData, 10)).map(c => c.id);
  ...
  const warehouseIds = (await insertInBatches('warehouses', warehousesData, 10)).map(w => w.id);
...
const userIds = (await insertInBatches('users', usersData, 3, 500, 3, 'id')).map(u => u.id);
  ...
  await insertInBatches('user_vehicles', vehiclesData, 10);
  ...
  await insertInBatches('password_reset_tokens', tokenData, 5);
  ...
  await insertInBatches('notifications', notifData, 10);
  console.log('✅ Notifications seeded.');

  // =========================================================================
  // 7. CATEGORIES & SUBCATEGORIES (20 родительских, 40 подкатегорий)
  // =========================================================================
  ...
  const parentIdsData = await insertInBatches('categories', parentInserts, 10, 400, 3, 'id, slug');
  ...
  for (const { slug } of parentCategories) {
    ...
    subs.forEach(sub => {
      subCatInserts.push({
        ...
        name: sub.name,
        slug: sub.slug,
        image_url: 'https://placehold.co/400',  // для подкатегорий можно заглушку
      });
    });
  }
  const insertedSubcats = await insertInBatches('categories', subCatInserts, 10, 400, 3, 'id, slug');
  ...
  // =========================================================================
  // 8. CATEGORY ATTRIBUTES (по 4 на родительскую категорию = 80)
  // =========================================================================
  const attrInserts = [];
  ...
  await insertInBatches('category_attributes', attrInserts, 10);
  console.log('✅ Category attributes seeded.');

  // =========================================================================
  // 9. BRANDS (24) с реальными логотипами из Storage
  // =========================================================================
  const brandsData = [
...
const insertedBrands = await insertInBatches('brands',
  ...
  // =========================================================================
  // 10. PRODUCTS (40) – файлы из твоих скриншотов
  // =========================================================================
  const productImages = [
    ...
    `${PRODUCTS_BUCKET}/CarShampooLiqui Moly.jpg`,            // пробел в имени файла
    ...
    `${PRODUCTS_BUCKET}/ImmobilizerGLA.jpg`,                  // из скриншота "ImmobilizerGLA.jpg"
    ...
    // Добавим два placeholder на случай нехватки
    `${PRODUCTS_BUCKET}/placeholder1.jpg`,
    `${PRODUCTS_BUCKET}/placeholder2.jpg`,
  ];
  const allProductImages = productImages; // готовый массив URL

  // Характеристики для каждого товара (подкатегории по порядку subSlugsOrder)
  const productDataBySubSlug = {
    'autoelectronics-alarms': {
      name: 'Автосигнализация StarLine A93',
      brand: 'StarLine',
      price: 12990, discount: 10990,
      chars: {
        'Тип': 'Двусторонняя',
        'Дальность': '2000 м',
        'Способ управления': 'Брелок',
        'Датчик удара': 'Есть',
        'Автозапуск': 'Да',
        'CAN-модуль': 'Интегрирован',
        'Напряжение питания': '12В',
        'Гарантия': '12 мес'
      }
    },
    'autoelectronics-multimedia': {
      name: 'Головное устройство Pioneer MVH-S120UB',
      brand: 'Pioneer',
      price: 5490, discount: null,
      chars: {
        'Тип': '1DIN',
        'Мощность': '4x50 Вт',
        'Поддержка USB': 'Да',
    ...
    'car-shampoos': {
      name: 'Автошампунь Liqui Moly 1л',
      brand: 'LiquiMoly',
      price: 590, discount: 490,
      chars: {
        'Объём': '1 л',
        'Тип': 'Концентрат',
        'pH': 'Нейтральный',
        'Запах': 'Лимон',
        'Назначение': 'Ручная мойка',
        'Безопасно для ЛКП': 'Да',
        'Разбавление': '1:100',
        'Страна': 'Германия'
      }
    },
    'car-polishes': {
      name: 'Полироль кузова Liqui Moly',
      brand: 'LiquiMoly',
      price: 1200, discount: 990,
      chars: {
        'Тип': 'Воск',
        'Объём': '250 мл',
        'Эффект': 'Глубокий блеск',
        'Защита': 'До 3 месяцев',
        'Применение': 'После мойки',
        'Ручное нанесение': 'Да',
        'Подходит для тёмных авто': 'Да',
        'Страна': 'Германия'
      }
    },
    'batteries-car': {
    ...
    'batteries-truck': {
      name: 'Аккумулятор грузовой Exide Premium',
      brand: 'Exide',
      price: 12500, discount: 11000,
      chars: {
        'Ёмкость': '180 Ач',
        'Пусковой ток': '1000 А',
        'Полярность': 'Прямая',
    ...
    'bodyparts-mirrors': {
      name: 'Зеркало с подогревом Alkar',
      brand: 'Leatherette',  // fallback
      price: 3200, discount: null,
      chars: {
        'Сторона': 'Левое',
        'Тип крепления': 'Ручное',
        'Подогрев': 'Да',
        'Цвет': 'Чёрный',
        'Материал корпуса': 'Пластик',
        'Совместимость': 'Lada Vesta',
        'Регулировка': 'Ручная',
        'Гарантия': '12 мес'
      }
    },
    'bodyparts-panels': {
      name: 'Кузовная панель порога',
      brand: 'EVA',
      price: 4500, discount: 3900,
      chars: {
        'Материал': 'Сталь',
        'Обработка': 'Катафорез',
        'Толщина металла': '0.8 мм',
        'Сторона': 'Левая',
        'Совместимость': 'Renault Logan',
        'Комплектация': '1 шт',
        'Вес': '2.3 кг',
        'Гарантия': '6 мес'
      }
    ...
    'brakes-discs': {
      ...
      brand: 'TRW',
      price: 3400, discount: 2990,
      chars: {
        'Диаметр': '280 мм',
        'Толщина': '22 мм',
        'Минимальная толщина': '20 мм',
        'Материал': 'Чугун',
        'Вентилируемый': 'Да',
        'Число отверстий': '5',
        'Ось': 'Передняя',
    ...
    'brakes-pads': {
      ...
      brand: 'Brembo',
      price: 2800, discount: 2400,
      chars: {
        'Материал': 'Керамика',
        'Коэффициент трения': '0.42',
        'Толщина': '17 мм',
        'Датчик износа': 'Есть',
        'Ось': 'Передняя',
        'Комплект': '4 шт',
    ...
    'cooling-radiators': {
      name: 'Радиатор Denso',
      brand: 'Denso',
      price: 8500, discount: 7900,
      chars: {
        'Тип': 'Алюминиевый',
        'Количество рядов': '2',
        'Размеры': '650x350x24 мм',
        'Применение': 'Охлаждение двигателя',
        'Конструкция': 'Сборный',
        'Вес': '4.5 кг',
        'Совместимость': 'Toyota Camry',
        'Гарантия': '18 мес'
      }
    },
    'cooling-thermostats': {
      ...
      brand: 'Mahle',
      price: 1200, discount: null,
      chars: {
        'Температура открытия': '87°C',
        'Диаметр': '54 мм',
        'Материал корпуса': 'Латунь',
        'Тип': 'С пружинным механизмом',
        'Высота': '80 мм',
    ...
    'engine-piston-rings': {
      name: 'Поршневые кольца Mahle',
      brand: 'Mahle',
      price: 3900, discount: 3500,
      chars: {
        'Комплект на': '4 цилиндра',
        'Диаметр': '82.5 мм',
        'Материал': 'Сталь/чугун',
        'Тип': 'Компрессионные + маслосъёмные',
        'Высота': '1.5/1.5/2.8 мм',
        'Зазор': '0.2-0.4 мм',
        'Применение': 'BMW N46',
        'Гарантия': '6 мес'
      }
    ...
    'engine-timing-kits': {
      name: 'Комплект ГРМ Bosch',
      brand: 'Bosch',
      price: 6700, discount: 6200,
      chars: {
        'Содержит': 'Ремень, ролики',
        'Ширина ремня': '25 мм',
        'Число зубьев': '143',
        'Материал ремня': 'Резина/стекловолокно',
        'Ролик натяжной': 'В комплекте',
        'Применение': 'Opel Astra J',
        'Интервал замены': '60 000 км',
        'Гарантия': '12 мес'
      }
    },
    'exhaust-mufflers': {
      name: 'Глушитель основной Bosal',
      brand: 'Bosch',  // fallback
      price: 5600, discount: null,
      chars: {
        'Тип': 'Основной',
        'Диаметр трубы': '55 мм',
        ...
        'Вес': '9 кг',
        'Применение': 'Ford Focus II',
        'Гарантия': '24 мес'
      }
    },
    'exhaust-catalytic': {
      name: 'Катализатор Walker',
      brand: 'Bosch',  // fallback
      price: 24300, discount: 22000,
      chars: {
        'Тип': 'Катализатор',
        'Евростандарт': 'Euro 5',
        'Диаметр трубы': '60 мм',
        'Материал корпуса': 'Нержавейка',
        'Количество сот': '400 cpsi',
        'Датчик кислорода': 'Совместим',
        'Применение': 'Renault Duster',
        'Гарантия': '12 мес'
      }
    },
    'filters-oil': {
      name: 'Масляный фильтр Mann',
      brand: 'Mahle',  // fallback
      price: 650, discount: null,
      chars: {
        'Тип': 'Масляный',
        'Резьба': 'M20x1.5',
        'Высота': '90 мм',
        'Диаметр корпуса': '65 мм',
        'Материал фильтроэлемента': 'Целлюлоза',
        'Пропускная способность': '18 л/мин',
    ...
    'filters-air': {
      ...
      chars: {
        ...
        'Высота': '45 мм',
        'Материал': 'Синтетика',
        'Класс фильтрации': '99.5%',
        'Применение': 'Hyundai Solaris',
        'Гарантия': '6 мес'
      }
    },
    'fuel-pumps': {
      ...
      brand: 'Bosch',
      price: 4300, discount: 3800,
      chars: {
        'Производительность': '120 л/ч',
        'Давление': '3.5 бар',
        'Тип': 'Погружной',
        'Материал': 'Алюминий/пластик',
        'Сетчатый фильтр': 'В комплекте',
        'Напряжение': '12В',
        'Применение': 'Lada Granta',
        'Гарантия': '18 мес'
      }
    },
    'fuel-injectors': {
      name: 'Форсунка дизельная Denso',
      brand: 'Denso',
      price: 8500, discount: 7900,
      chars: {
        'Тип': 'Электромагнитная',
        'Сопротивление': '0.5 Ом',
        'Давление открытия': '1600 бар',
        'Количество отверстий': '7',
        'Распылитель': 'Sac',
        'Кодировка': 'С2',
        'Применение': 'Common Rail',
        'Гарантия': '12 мес'
      }
    },
    'ignition-plugs': {
      ...
      chars: {
        ...
        'Применение': 'Бензиновые ДВС',
        'Гарантия': '12 мес'
      }
    },
    'ignition-coils': {
      name: 'Катушка зажигания Bosch',
      brand: 'Bosch',
      price: 2200, discount: 1900,
      chars: {
        'Тип': 'Индивидуальная',
        'Сопротивление первичной обмотки': '0.7 Ом',
        'Вторичной обмотки': '8 кОм',
        'Выходное напряжение': '40 кВ',
        'Материал корпуса': 'Эпоксидный компаунд',
        'Совместимость': 'VAG',
        'Длина кабеля': '120 мм',
        'Гарантия': '24 мес'
      }
    ...
    'interior-seat-covers': {
      name: 'Чехлы на сидения Leatherette',
      brand: 'Leatherette',
      price: 6500, discount: 5700,
      chars: {
        'Материал': 'Экокожа',
        'Цвет': 'Чёрный',
        'Количество мест': '5',
        'Крепление': 'Липучки/крючки',
        'Водонепроницаемость': 'Да',
        'Совместимость': 'Универсальные',
        'Подогрев': 'Совместимы',
        'Гарантия': '12 мес'
      }
    },
    'interior-floor-mats': {
      name: 'Коврики салона EVA',
      brand: 'EVA',
      price: 2100, discount: 1800,
      chars: {
        'Материал': 'EVA',
        'Цвет': 'Серый',
        'Количество': '4 шт',
    ...
    'lighting-headlights': {
      name: 'Фара Bosch',
      brand: 'Bosch',
      price: 12400, discount: 11500,
      chars: {
        'Тип лампы': 'H7',
        'Регулировка': 'Электрическая',
        'Материал стекла': 'Поликарбонат',
        'Сторона': 'Левая',
        'Габариты': '500x300x250 мм',
        'Применение': 'Skoda Octavia A7',
        'Омыватель': 'Совместима',
        'Гарантия': '24 мес'
      }
    },
    'lighting-xenon': {
      name: 'Ксеноновая лампа Philips',
      brand: 'Pioneer',  // fallback (Philips нет в списке брендов)
      price: 2800, discount: null,
      chars: {
        'Цоколь': 'D2S',
        'Цветовая температура': '4300K',
        'Световой поток': '3200 люмен',
        'Мощность': '35 Вт',
        'Срок службы': '2500 ч',
        'Тип': 'Газоразрядная',
        'Блок розжига': 'Не входит',
        'Гарантия': '12 мес'
      }
    },
    'oils-engine': {
      ...
      brand: 'Mobil1',
      price: 3800, discount: 3400,
      chars: {
        'Объём': '4 л',
        'Вязкость': '5W-30',
        'Тип': 'Синтетическое',
        'Спецификации': 'API SN, ACEA A3/B4',
        'Температура застывания': '-42°C',
        'Назначение': 'Бензиновые/дизельные',
        'Страна': 'США',
        'Гарантия': 'Нет'
      }
    },
    'oils-antifreeze': {
      name: 'Антифриз Liqui Moly G11',
      brand: 'LiquiMoly',
      price: 1100, discount: 950,
      chars: {
        'Объём': '5 л',
        'Цвет': 'Зелёный',
        'Тип': 'G11',
        'Температура кипения': '108°C',
        'Защита от коррозии': 'Да',
        'Срок службы': '3 года',
        'Концентрат': 'Готовый к применению',
        'Страна': 'Германия'
      }
    },
    'security-immobilizers': {
      name: 'Иммобилайзер IGLA',
      brand: 'IGLA',
      price: 8900, discount: 7900,
      chars: {
        'Тип': 'Цифровой',
        'Защита': 'От угона',
        'Способ активации': 'Метка',
        'Дистанция считывания': '2 м',
        'Режим охраны': 'Автоматический',
        'Блокировка двигателя': 'Цифровая',
        'Напряжение': '12В',
        'Гарантия': '12 мес'
      }
    },
    'security-alarms': {
      ...
      chars: {
        ...
        'Напряжение': '12В',
        'Гарантия': '12 мес',
        'Доп. функции': 'Bluetooth'
      }
    },
    'steering-racks': {
      name: 'Рулевая рейка Bosch',
      brand: 'Bosch',
      price: 18900, discount: 17500,
      chars: {
        'Тип': 'Реечная',
        'Передаточное число': '3.2',
        'Рабочий ход': '140 мм',
        'Обслуживание': 'Необслуживаемая',
        'Материал': 'Сталь/алюминий',
        'Применение': 'Toyota Corolla',
        'Вес': '8 кг',
        'Гарантия': '24 мес'
      }
    },
    'steering-tie-rods': {
      name: 'Рулевая тяга TRW',
      brand: 'TRW',
      price: 1900, discount: 1700,
      chars: {
        'Длина': '280 мм',
        'Резьба': 'M14x1.5',
        'Материал': 'Сталь',
    ...
    'suspension-shocks': {
      ...
      brand: 'KYB',
      price: 4700, discount: 4200,
      chars: {
        'Тип': 'Масляный',
        'Диаметр штока': '22 мм',
        'Длина': '520 мм',
        'Регулировка жёсткости': 'Нет',
        'Применение': 'Передний',
        'Кузов': 'Седан',
        'Совместимость': 'Ford Focus',
        'Гарантия': '24 мес'
      }
    },
    'suspension-arms': {
      name: 'Рычаг подвески Lemforder',
      brand: 'Bosch',  // fallback
      price: 5600, discount: null,
      chars: {
    ...
    'tires': {
      name: 'Шина летняя Michelin Primacy 4',
      brand: 'Michelin',
      price: 8500, discount: 7800,
      chars: {
        'Размер': '205/55 R16',
        'Сезон': 'Лето',
        'Индекс скорости': 'V',
        ...
        'Топливная экономичность': 'B',
        'Сцепление на мокрой дороге': 'A',
        'Шум': '69 дБ'
      }
    },
    'wheels': {
      name: 'Диск колесный Continental 6.5x16',
      brand: 'Continental',
      price: 7400, discount: null,
      chars: {
        'Диаметр': '16 дюймов',
        'Ширина': '6.5J',
        'PCD': '5x112',
        'Вылет ET': '45',
        'Материал': 'Легкосплавный',
        'Цвет': 'Серебристый',
        'Вес': '8.9 кг',
        'Гарантия': '12 мес'
      }
    ...
    'tools-jacks': {
      name: 'Домкрат подкатной TRW',
      brand: 'TRW',
      price: 3200, discount: 2900,
      chars: {
        'Грузоподъёмность': '2.5 т',
        'Высота подхвата': '85-375 мм',
        'Тип': 'Гидравлический подкатной',
        'Материал': 'Сталь',
        'Вес': '12 кг',
        'Ручка': '2 секции',
        'Колёса': 'Поворотные',
    ...
    'tools-sets': {
      name: 'Набор инструментов Bosch 108 предметов',
      brand: 'Bosch',
      price: 5900, discount: 5400,
      chars: {
        'Количество предметов': '108',
        'Привод': '1/4", 1/2"',
        'Трещотки': '2 шт',
    ...
    'transmission-clutch': {
      name: 'Комплект сцепления Sachs',
      brand: 'Bosch',  // fallback
      price: 8700, discount: 7900,
      chars: {
        'Тип': 'Сухое, однодисковое',
        'Диаметр диска': '228 мм',
        'Ступица': '23 шлица',
        'Ведомый диск': 'С пружинами',
        'Корзина': 'Диафрагменная',
        'Применение': 'VW Golf VI',
        'Вес комплекта': '7.2 кг',
        'Гарантия': '12 мес'
      }
    },
    'transmission-cvjoints': {
      name: 'ШРУС наружный GKN Loebro',
      brand: 'TRW',  // fallback
      price: 3900, discount: 3500,
      chars: {
        'Тип': 'Наружный',
        'Количество шлицов': '27',
        'Диаметр': '88 мм',
        'Материал': 'Сталь',
        'Пыльник': 'В комплекте',
        'Смазка': 'Литиевая',
        'Применение': 'Renault Logan',
        'Гарантия': '12 мес'
      }
    },
  };

  // Порядок slug'ов подкатегорий в соответствии с вставкой
  const subSlugsOrder = [];
  ...
  subSlugsOrder.forEach((slug, i) => {
    ...
    const imageUrl = allProductImages[i] || `${PRODUCTS_BUCKET}/placeholder.jpg`;
    let tags = [];
    ...
    productInserts.push({
      ...
      images: [imageUrl],              // массив с одним URL
      tags,
  ...
  const insertedProducts = await insertInBatches('products', productInserts, 10);
  ...
  // =========================================================================
  const stockInserts = productIds.map(prodId => ({
    product_id: prodId,
    warehouse_id: warehouseIds[rand(0, warehouseIds.length-1)],
    quantity: rand(1, 50),
    shelf_location: `R${rand(1,20)}-S${rand(1,10)}`,
  }));
  await insertInBatches('product_stocks', stockInserts, 10);
  ...
  const orderIds = (await insertInBatches('orders', orderInserts, 10)).map(o => o.id);
  ...
  await insertInBatches('order_items', itemInserts, 10);
  console.log('✅ Order items seeded.');

  // =========================================================================
  // 14. ORDER STATUS HISTORY (5)
  // =========================================================================
  ...
  await insertInBatches('order_status_history', statusInserts, 5);
  ...
  await insertInBatches('reviews', reviewInserts, 10);
  ...
  try {
    await insertInBatches('wishlists', wishInserts, 10);
    console.log('✅ Wishlists seeded.');
  } catch (e) {
    console.warn('⚠️ Wishlists (возможно, дубликаты) проигнорированы:', e.message);
  }

  // =========================================================================
  // 17. RETURN REQUESTS (5)
  // =========================================================================
  ...
  await insertInBatches('return_requests', returnInserts, 5);
